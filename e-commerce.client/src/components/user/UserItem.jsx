...
const UserItem = ({ user, isItemOnBasket, addToBasket }) => {
  ...
  const handleAddToBasket = () => {
    if (addToBasket) addToBasket({ ...user, selectedSize: user.sizes[0] });
  };

  return (
    <SkeletonTheme color="#e1e1e1" highlightColor="#f2f2f2">
      <div
        className={`product-card ${!user.id ? 'product-loading' : ''}`}
        style={{
        ...
        >
          ...
          <div className="product-details">
            <h5 className="product-card-name text-overflow-ellipsis margin-auto">
              {user.fullname || <Skeleton width={80} />}
            </h5>
            <p className="product-card-brand">
              {user.email || <Skeleton width={60} />}
            </p>
            <p className="product-card-brand">
              {user.address || <Skeleton width={60} />}
            </p>
            <p className="product-card-brand">
              {user.role || <Skeleton width={60} />}
            </p>
